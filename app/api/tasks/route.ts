...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to fetch tasks') }, { status: 500 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to create task') }, { status: 500 })
    }
