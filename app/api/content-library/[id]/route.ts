...
export async function GET(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: clientMessage(error, 'Failed to load content') }, { status: 500 })
    }
...
export async function PUT(
  ...
  try {
    ...
    if (contentError) {
      ...
      return NextResponse.json({ error: clientMessage(contentError, 'Failed to update content') }, { status: 500 })
    }
