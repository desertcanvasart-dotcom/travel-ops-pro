...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: clientMessage(error, 'Failed to fetch activity') }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: clientMessage(error, 'Failed to log activity') }, { status: 500 })
  }
}