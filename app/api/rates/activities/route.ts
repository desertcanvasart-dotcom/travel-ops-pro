...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to load activity rates') }, { status: 500 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to save activity rate') }, { status: 500 })
    }
