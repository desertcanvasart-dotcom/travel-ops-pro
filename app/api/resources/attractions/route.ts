...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to fetch attraction rates') }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to create attraction rate') }, { status: 500 })
  }
}