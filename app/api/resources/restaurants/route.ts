...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to fetch restaurant rates') }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: clientMessage(error, 'Failed to create restaurant rate') }, { status: 500 })
  }
}