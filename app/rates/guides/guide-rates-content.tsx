...
export default function GuideRatesContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.data) {
        ...
        showToast('error', 'No data returned')
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string, language: string) => {
    if (!confirm(`Delete rate for ${language}?`)) return

    try {
      const response = await fetch(`/api/rates/guides/${id}`, {
        method: 'DELETE'
      })
                  ...
                  <tr key={rate.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          onClick={() => handleDelete(rate.id, rate.guide_language)}
              ...
              <div key={rate.id} className="px-4 py-2 flex items-center justify-between hover:bg-gray-50">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => handleDelete(rate.id, rate.guide_language)} className="p-1 text-gray-400 hover:text-red-600">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
