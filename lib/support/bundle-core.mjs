...
export function bundleFindings(bundle) {
  ...
  if (scheduledHere.length > 0 && neverRunScheduled.length === scheduledHere.length) {
    // Only prescribe the variable when it is actually unset. On an install
    // that has it set — a fresh one, where the first daily job is simply hours
    // away — telling the operator to set what they already set is the same
    // mistake as reporting a working install as broken, and it teaches them to
    // stop reading these.
    const armed = bundle.env.set.includes('CRON_IN_PROCESS')
    findings.push(
      armed
        ? 'No scheduled job has ever run here yet. CRON_IN_PROCESS is set, so the scheduler arms itself at startup; on an install that has just been stood up the first daily job has simply not come round. Check again after a day — a job with no run by then has never run.'
