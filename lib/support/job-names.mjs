...
// task-reminders and dispatch-scheduled-sends left no trace whatsoever: if
...
// 2026-09-23: the three external jobs (pg_cron in the production database) now
// run in-process, and dispatch-scheduled-sends was removed with the rest of the
// never-finished scheduled-sends feature (its table was never created in prod).
// Every job below is now scheduled by lib/cron/scheduler.ts.
...
export const SCHEDULED_IN_PROCESS = [
  ...
  'gmail-sync',
  'survey-invites',
  // Moved in from a pg_cron job in the production database (2026-09-23).
  'refresh-exchange-rates',
  'send-reminders',
  'task-reminders',
]
