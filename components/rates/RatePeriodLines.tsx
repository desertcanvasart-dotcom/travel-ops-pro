...
/** A rate row that predates periods (or came in through the wide importer
 *  with no season dates) prices from its base columns — so the list shows
 *  those as one undated line rather than "No periods entered" beside a rate
 *  that does price (Greptile on #452). Null when nothing is priced there. */
export function baseColumnsAsPeriod(row: object, entity: RateSeasonEntity): RateSeason | null {
  const r = row as Record<string, unknown>
  const n = (v: unknown) => Number(v) || 0
  // The same columns resolveHotelRatesForDate / resolveCruiseRatesForDate read.
  const rates: Record<string, number> = entity === 'accommodation'
    ? {
        pp_double_eur: n(r.pp_double_eur), single_supp_eur: n(r.single_supp_eur), triple_red_eur: n(r.triple_red_eur),
        pp_double_non_eur: n(r.pp_double_non_eur), single_supp_non_eur: n(r.single_supp_non_eur), triple_red_non_eur: n(r.triple_red_non_eur),
      }
    : {
        double_eur: n(r.rate_low_double_eur) || n(r.rate_double_eur),
        single_eur: n(r.rate_low_single_eur) || n(r.rate_single_eur),
        triple_eur: n(r.rate_low_triple_eur) || n(r.rate_triple_eur),
        double_non_eur: n(r.rate_low_double_non_eur),
        single_non_eur: n(r.rate_low_single_non_eur),
        triple_non_eur: n(r.rate_low_triple_non_eur),
      }
  const priced = Object.values(rates).some(v => v > 0)
  return priced ? { name: '', from: '', to: '', rates } : null
}

/** The periods a list shows for a row: its dated periods, else its base
 *  columns as one undated line, else nothing. */
export function displayPeriods(row: object, entity: RateSeasonEntity): { periods: RateSeason[]; undated: boolean } {
  const seasons = seasonsForRow(row, entity)
  if (seasons.length) return { periods: seasons, undated: false }
  const base = baseColumnsAsPeriod(row, entity)
  return { periods: base ? [base] : [], undated: Boolean(base) }
}
...
export function pricedDoubles(
  ...
  return rows.flatMap(row => {
    ...
    return displayPeriods(row, entity).periods.flatMap(season =>
...
export default function RatePeriodLines({ row, entity, format, compact = false }: Props) {
  ...
  const { periods: seasons, undated } = displayPeriods(row, entity)
        ...
        return (
          <div key={`${season.from}-${i}`} className={`flex ${compact ? 'flex-col' : 'items-start'} gap-2 border-t border-gray-100 pt-1.5 first:border-0 first:pt-0`} data-testid="rate-period-line">
            <div className={compact ? '' : 'w-48 shrink-0'}>
              {undated ? (
                <>
                  <p className="font-medium text-gray-900">{t('baseRate')}</p>
                  <p className="text-[11px] text-amber-700">{t('baseRateHint')}</p>
                </>
              ) : (
                <>
