-- 20260918_supplier_type_reseed.sql
-- Reconcile the supplier_type vocabulary to the app's REAL supplier roles.
--
-- The 20260908 seed carried a 14-key supplier_type taxonomy from the sibling
-- SaaS (transport_company, airline, …) that never matched this app: the
-- suppliers page is driven by a hardcoded TYPE_CONFIG of 17 roles (hotel,
-- transport, local_operator, air_carrier, airport_assistant, hotel_assistant,
-- …), the supplier form offers exactly those, and suppliers.types[] stores
-- them. The vocab was disconnected from reality — nothing read it for the form.
--
-- This migration:
--   1. CREATE OR REPLACEs seed_org_vocabulary with the app's 17 real role keys,
--      each carrying its behavior (the built-in pricing/routing kind it maps to)
--      and bilingual labels. behaviors: local_operator -> tour_operator;
--      airport_assistant & hotel_assistant -> ground_handler; the rest map by
--      name (transport -> transport_company, air_carrier -> airline).
--   2. Re-seeds existing orgs: deletes their (wrong-key) supplier_type rows and
--      re-runs the seed. Safe because the supplier FORM reads TYPE_CONFIG, not
--      this vocab — no suppliers.types data depends on these rows.
--
-- Relabel-only for the UI: suppliers.types keys are frozen (TYPE_CONFIG); this
-- supplies the DISPLAY labels + behavior metadata.
...
-- Re-seed existing orgs' supplier_type rows with the corrected 17 keys.
DO $reseed$
DECLARE
  org RECORD;
BEGIN
  DELETE FROM org_vocabularies WHERE kind = 'supplier_type';
  FOR org IN SELECT id FROM public.organizations LOOP
    PERFORM seed_org_vocabulary(org.id, 'supplier_type');
  END LOOP;
END
$reseed$;

-- Self-verify: exactly the 17 app keys, all bilingual, all with a valid behavior.
DO $verify$
DECLARE
  expected TEXT[] := ARRAY[
    'hotel','transport','local_operator','driver','guide','cruise',
    'activity_provider','attraction','tour_operator','ground_handler',
    'train_operator','air_carrier','airport_assistant','hotel_assistant',
    'restaurant','shop','other'
  ];
  valid_beh TEXT[] := ARRAY[
    'hotel','transport_company','airline','train_operator','driver','guide',
    'cruise','activity_provider','attraction','tour_operator','ground_handler',
    'restaurant','shop','other'
  ];
  bad INTEGER;
  missing INTEGER;
...
BEGIN
  ...
  WHERE kind = 'supplier_type' AND key <> ALL(expected);
  IF bad > 0 THEN
    RAISE EXCEPTION 'supplier_type reseed: % stray rows with unexpected keys', bad;
  END IF;
  ...
  WHERE EXISTS (SELECT 1 FROM org_vocabularies v WHERE v.org_id = o.id)
    AND NOT EXISTS (
      ...
      WHERE v.org_id = o.id AND v.kind = 'supplier_type' AND v.key = k.key
    );
  IF missing > 0 THEN
    RAISE EXCEPTION 'supplier_type reseed: % missing (org,key) pairs', missing;
  END IF;

  SELECT count(*) INTO no_ja
  FROM org_vocabularies
  WHERE kind = 'supplier_type' AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'supplier_type reseed: % rows without a Japanese label', no_ja;
  END IF;

  SELECT count(*) INTO bad_beh
  FROM org_vocabularies
  WHERE kind = 'supplier_type' AND (behavior IS NULL OR behavior <> ALL(valid_beh));
  IF bad_beh > 0 THEN
    RAISE EXCEPTION 'supplier_type reseed: % rows with an invalid behavior', bad_beh;
  END IF;
END
$verify$;
