-- 20260923_attraction_fee_type_relabel.sql
-- Bilingual backfill for the attraction_fee_type vocabulary.
--
-- Keys + English labels already match the app (the attractions page
-- feeTypeOptions, rendered via rates.attractions.feeTypes i18n). The seed left
-- label_ja NULL; the Japanese labels come from that same i18n block. English
-- is unchanged.
--
-- Built from 20260922 so all prior fixes are retained.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = '標準'
    WHERE kind = 'attraction_fee_type' AND key = 'standard' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '入場無料'
    WHERE kind = 'attraction_fee_type' AND key = 'free' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '寄付ベース'
    WHERE kind = 'attraction_fee_type' AND key = 'donation' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'パッケージに含む'
    WHERE kind = 'attraction_fee_type' AND key = 'included' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: all four fee types must be bilingual after the backfill.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind = 'attraction_fee_type'
    AND key IN ('standard','free','donation','included')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'attraction_fee_type relabel: % fee types without a Japanese label', no_ja;
  END IF;
END
$verify$;
