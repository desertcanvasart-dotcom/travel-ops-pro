-- 20260921_tipping_kinds_relabel.sql
-- Reconcile tipping_role's key + bilingual backfill for all three tipping kinds.
--
-- tipping_context and tipping_unit keys + English labels already match the app
-- (rates.tipping CONTEXTS / RATE_UNITS, rendered via i18n). tipping_role matches
-- EXCEPT one key: the 20260908 seed used 'hotel_staff', but the app (ROLE_TYPES
-- and rates.tipping.roleTypes i18n) uses 'hotel_assistant'. All three seeds left
-- label_ja NULL. Japanese labels here come straight from the app's own i18n.
--
-- Built from 20260920 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with tipping_role's key corrected
--      to 'hotel_assistant' + label_ja on all three tipping blocks.
--   2. Existing orgs: rename the 'hotel_staff' row to 'hotel_assistant' (only
--      where the target doesn't already exist), then backfill label_ja where
--      blank — preserving customization.
--
-- Relabel-only otherwise: the keys are frozen; this supplies labels.
...
-- Rename existing orgs' tipping_role 'hotel_staff' -> 'hotel_assistant'.
UPDATE org_vocabularies v SET key = 'hotel_assistant', label = 'Hotel Assistant'
...
-- Backfill Japanese labels where blank.
  ...
  UPDATE org_vocabularies SET label_ja = 'ドライバー'
    WHERE kind = 'tipping_role' AND key = 'driver' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '船員'
    WHERE kind = 'tipping_role' AND key = 'boat_crew' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ポーター'
    WHERE kind = 'tipping_role' AND key = 'porter' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ホテルアシスタント'
    WHERE kind = 'tipping_role' AND key = 'hotel_assistant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'レストラン'
    WHERE kind = 'tipping_role' AND key = 'restaurant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'その他'
    WHERE kind = 'tipping_role' AND key = 'other' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '日帰りツアー'
    WHERE kind = 'tipping_context' AND key = 'day_tour' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '半日ツアー'
    WHERE kind = 'tipping_context' AND key = 'half_day_tour' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'クルーズ'
    WHERE kind = 'tipping_context' AND key = 'cruise' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '送迎'
    WHERE kind = 'tipping_context' AND key = 'transfer' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '空港'
    WHERE kind = 'tipping_context' AND key = 'airport' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ホテル'
    WHERE kind = 'tipping_context' AND key = 'hotel' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'レストラン'
    WHERE kind = 'tipping_context' AND key = 'restaurant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ファルーカ'
    WHERE kind = 'tipping_context' AND key = 'felucca' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'モーターボート'
    WHERE kind = 'tipping_context' AND key = 'motorboat' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1日あたり'
    WHERE kind = 'tipping_unit' AND key = 'per_day' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1サービスあたり'
    WHERE kind = 'tipping_unit' AND key = 'per_service' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1クルーズあたり'
    WHERE kind = 'tipping_unit' AND key = 'per_cruise' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1泊あたり'
    WHERE kind = 'tipping_unit' AND key = 'per_night' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1名あたり'
    WHERE kind = 'tipping_unit' AND key = 'per_person' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: no stale 'hotel_staff' key, and every tipping row is bilingual.
DO $verify$
DECLARE
  stale INTEGER;
  no_ja INTEGER;
BEGIN
  ...
  FROM org_vocabularies WHERE kind = 'tipping_role' AND key = 'hotel_staff';
  IF stale > 0 THEN
    RAISE EXCEPTION 'tipping relabel: % rows still keyed hotel_staff', stale;
  END IF;

  SELECT count(*) INTO no_ja
  FROM org_vocabularies
  WHERE kind IN ('tipping_role','tipping_context','tipping_unit')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'tipping relabel: % rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
