-- 20260924_activity_kinds_relabel.sql
-- Bilingual backfill for the four activity vocabularies:
--   activity_type, activity_duration, activity_unit, activity_pricing_type.
--
-- Keys + English labels already match the app (the activities rate page
-- ACTIVITY_TYPES / DURATIONS / UNIT_LABELS / PRICING_TYPES arrays). The seed
-- left label_ja NULL; this supplies the Japanese. English is unchanged, with
-- one reconciliation called out in the PR: the pricing-type labels converge on
-- the concise forms already in the seed ('Per Unit', 'Tiered') — the form
-- button previously read 'Per Unit'/'Tiered (Volume Discount)' and the filter
-- 'Per Unit (Boat/Ride)'/'Tiered', so the badge, filter and button now agree.
--
-- Built from 20260923 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on the four blocks.
...
-- Backfill Japanese labels where blank — preserving any customization.
  -- activity_type
  UPDATE org_vocabularies SET label_ja = 'ガイド付きツアー'
    WHERE kind = 'activity_type' AND key = 'guided_tour' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'セルフガイド'
    WHERE kind = 'activity_type' AND key = 'self_guided' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エクスカーション'
    WHERE kind = 'activity_type' AND key = 'excursion' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '日帰り旅行'
    WHERE kind = 'activity_type' AND key = 'day_trip' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '半日旅行'
    WHERE kind = 'activity_type' AND key = 'half_day_trip' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '体験'
    WHERE kind = 'activity_type' AND key = 'experience' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ワークショップ'
    WHERE kind = 'activity_type' AND key = 'workshop' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ショー'
    WHERE kind = 'activity_type' AND key = 'show' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'クルーズ'
    WHERE kind = 'activity_type' AND key = 'cruise' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '乗り物'
    WHERE kind = 'activity_type' AND key = 'ride' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'アクティビティ'
    WHERE kind = 'activity_type' AND key = 'activity' AND coalesce(btrim(label_ja), '') = '';

  -- activity_duration
  UPDATE org_vocabularies SET label_ja = '30分'
    WHERE kind = 'activity_duration' AND key = '30_minutes' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1時間'
    WHERE kind = 'activity_duration' AND key = '1_hour' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '1.5時間'
    WHERE kind = 'activity_duration' AND key = '1_5_hours' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '2時間'
    WHERE kind = 'activity_duration' AND key = '2_hours' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '3時間'
    WHERE kind = 'activity_duration' AND key = '3_hours' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '4時間'
    WHERE kind = 'activity_duration' AND key = '4_hours' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '半日（4〜5時間）'
    WHERE kind = 'activity_duration' AND key = 'half_day_4_5h' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '終日（6〜8時間）'
    WHERE kind = 'activity_duration' AND key = 'full_day_6_8h' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '延長日（8〜10時間）'
    WHERE kind = 'activity_duration' AND key = 'extended_day_8_10h' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '複数日'
    WHERE kind = 'activity_duration' AND key = 'multi_day' AND coalesce(btrim(label_ja), '') = '';

  -- activity_unit
  UPDATE org_vocabularies SET label_ja = 'ボート'
    WHERE kind = 'activity_unit' AND key = 'boat' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ファルーカ'
    WHERE kind = 'activity_unit' AND key = 'felucca' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '乗り物'
    WHERE kind = 'activity_unit' AND key = 'ride' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'バギー'
    WHERE kind = 'activity_unit' AND key = 'quad' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '車両'
    WHERE kind = 'activity_unit' AND key = 'vehicle' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'チケット'
    WHERE kind = 'activity_unit' AND key = 'ticket' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'グループ'
    WHERE kind = 'activity_unit' AND key = 'group' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'セッション'
    WHERE kind = 'activity_unit' AND key = 'session' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '名'
    WHERE kind = 'activity_unit' AND key = 'person' AND coalesce(btrim(label_ja), '') = '';

  -- activity_pricing_type
  UPDATE org_vocabularies SET label_ja = '1人当たり'
    WHERE kind = 'activity_pricing_type' AND key = 'per_person' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ユニット単位'
    WHERE kind = 'activity_pricing_type' AND key = 'per_unit' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '定額料金'
    WHERE kind = 'activity_pricing_type' AND key = 'flat' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '段階料金'
    WHERE kind = 'activity_pricing_type' AND key = 'tiered' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded row of the four activity kinds must be bilingual
-- after the backfill (rows an operator added themselves are exempt).
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE (
      (kind = 'activity_type' AND key IN ('guided_tour','self_guided','excursion','day_trip','half_day_trip','experience','workshop','show','cruise','ride','activity'))
   OR (kind = 'activity_duration' AND key IN ('30_minutes','1_hour','1_5_hours','2_hours','3_hours','4_hours','half_day_4_5h','full_day_6_8h','extended_day_8_10h','multi_day'))
   OR (kind = 'activity_unit' AND key IN ('boat','felucca','ride','quad','vehicle','ticket','group','session','person'))
   OR (kind = 'activity_pricing_type' AND key IN ('per_person','per_unit','flat','tiered'))
    )
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'activity kinds relabel: % activity rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
