-- 20260916_restaurant_cuisine_relabel.sql
-- Bilingual backfill for the restaurant_type and cuisine_type vocabularies.
--
-- Both kinds' keys + English labels already match the app (the meals page
-- RESTAURANT_TYPES / CUISINE_TYPES store raw strings, reached by slugify), but
-- the page has no i18n for them, so the Japanese UI shows English, and the seed
-- left label_ja NULL.
--   * restaurant_type label_ja comes from the app's own rates.meals i18n.
--   * cuisine_type had no app i18n at all — label_ja here are the conventional
--     Japanese cuisine names, which the operator can edit in the Vocabulary UI.
-- English is unchanged, so this only ever adds a Japanese label.
--
-- Built from 20260915 so all prior fixes are retained.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = 'ファインダイニング'
    WHERE kind = 'restaurant_type' AND key = 'fine_dining' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'カジュアルダイニング'
    WHERE kind = 'restaurant_type' AND key = 'casual_dining' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ビュッフェ'
    WHERE kind = 'restaurant_type' AND key = 'buffet' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ローカルレストラン'
    WHERE kind = 'restaurant_type' AND key = 'local_restaurant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ホテルレストラン'
    WHERE kind = 'restaurant_type' AND key = 'hotel_restaurant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'クルーズレストラン'
    WHERE kind = 'restaurant_type' AND key = 'cruise_restaurant' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ストリートフード'
    WHERE kind = 'restaurant_type' AND key = 'street_food' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'カフェ'
    WHERE kind = 'restaurant_type' AND key = 'cafe' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エジプト料理'
    WHERE kind = 'cuisine_type' AND key = 'egyptian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '地中海料理'
    WHERE kind = 'cuisine_type' AND key = 'mediterranean' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'イタリア料理'
    WHERE kind = 'cuisine_type' AND key = 'italian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '中東料理'
    WHERE kind = 'cuisine_type' AND key = 'middle_eastern' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'アジア料理'
    WHERE kind = 'cuisine_type' AND key = 'asian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'インターナショナル'
    WHERE kind = 'cuisine_type' AND key = 'international' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'シーフード'
    WHERE kind = 'cuisine_type' AND key = 'seafood' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'レバノン料理'
    WHERE kind = 'cuisine_type' AND key = 'lebanese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'トルコ料理'
    WHERE kind = 'cuisine_type' AND key = 'turkish' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'インド料理'
    WHERE kind = 'cuisine_type' AND key = 'indian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '韓国料理'
    WHERE kind = 'cuisine_type' AND key = 'korean' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '中華料理'
    WHERE kind = 'cuisine_type' AND key = 'chinese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '日本料理'
    WHERE kind = 'cuisine_type' AND key = 'japanese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フランス料理'
    WHERE kind = 'cuisine_type' AND key = 'french' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded restaurant_type + cuisine_type row must be bilingual.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind IN ('restaurant_type','cuisine_type')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'restaurant/cuisine relabel: % rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
