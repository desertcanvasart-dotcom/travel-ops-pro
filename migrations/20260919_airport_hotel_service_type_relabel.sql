-- 20260919_airport_hotel_service_type_relabel.sql
-- Bilingual backfill for the airport_service_type and hotel_service_type vocabs.
--
-- Both kinds' keys + English labels already match the app (the airport-services
-- and hotel-services rate pages store SERVICE_TYPES slugs and render them via
-- i18n rates.airportServices/hotelServices.serviceTypes). The seed left
-- label_ja NULL. The Japanese labels here come straight from those same i18n
-- blocks, so nothing is invented and English is unchanged.
--
-- Built from 20260918 so all prior fixes are retained.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = 'ミート&グリート'
    WHERE kind = 'airport_service_type' AND key = 'meet_greet' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '税関アシスト'
    WHERE kind = 'airport_service_type' AND key = 'customs_assist' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フルサービス'
    WHERE kind = 'airport_service_type' AND key = 'full_service' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'VIPサービス'
    WHERE kind = 'airport_service_type' AND key = 'vip_service' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ポーター'
    WHERE kind = 'hotel_service_type' AND key = 'porter' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'チェックインアシスト'
    WHERE kind = 'hotel_service_type' AND key = 'checkin_assist' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'チェックアウトアシスト'
    WHERE kind = 'hotel_service_type' AND key = 'checkout_assist' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フルサービス'
    WHERE kind = 'hotel_service_type' AND key = 'full_service' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'コンシェルジュ'
    WHERE kind = 'hotel_service_type' AND key = 'concierge' AND coalesce(btrim(label_ja), '') = '';
...
BEGIN
  ...
  WHERE kind IN ('airport_service_type','hotel_service_type')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'airport/hotel service_type relabel: % rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
