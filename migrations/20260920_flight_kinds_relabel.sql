-- 20260920_flight_kinds_relabel.sql
-- Bilingual backfill for flight_type, flight_cabin and flight_frequency.
--
-- All three kinds' keys + English labels already match the app (the flights
-- rate page's FLIGHT_TYPES / CABIN_CLASSES / FREQUENCIES arrays, rendered from
-- their hardcoded .label). The seed left label_ja NULL. Japanese labels come
-- from rates.flights i18n where present; flight_cabin 'first' uses the
-- firstClass label (ファーストクラス), not the bare 'first' i18n key (最初 =
-- ordinal); the frequency values with no i18n use the conventional Japanese.
-- English is unchanged.
--
-- Built from 20260919 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on all three blocks.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = '国内線'
    WHERE kind = 'flight_type' AND key = 'domestic' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '国際線'
    WHERE kind = 'flight_type' AND key = 'international' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エコノミー'
    WHERE kind = 'flight_cabin' AND key = 'economy' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ビジネス'
    WHERE kind = 'flight_cabin' AND key = 'business' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ファーストクラス'
    WHERE kind = 'flight_cabin' AND key = 'first' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '毎日'
    WHERE kind = 'flight_frequency' AND key = 'daily' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '平日のみ'
    WHERE kind = 'flight_frequency' AND key = 'weekdays' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '週末のみ'
    WHERE kind = 'flight_frequency' AND key = 'weekends' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '月・水・金'
    WHERE kind = 'flight_frequency' AND key = 'mon_wed_fri' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '火・木・土'
    WHERE kind = 'flight_frequency' AND key = 'tue_thu_sat' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '週1回'
    WHERE kind = 'flight_frequency' AND key = 'weekly' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'チャーター／随時'
    WHERE kind = 'flight_frequency' AND key = 'charter' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded row of all three kinds must be bilingual.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind IN ('flight_type','flight_cabin','flight_frequency')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'flight kinds relabel: % rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
