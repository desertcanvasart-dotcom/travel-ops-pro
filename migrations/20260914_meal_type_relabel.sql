-- 20260914_meal_type_relabel.sql
-- Bilingual backfill for the meal_type vocabulary.
--
-- meal_type keys + English labels already match the app: the meals page stores
-- and shows the raw strings 'Breakfast'/'Full Board'/… (MEAL_TYPES), reached by
-- slugifying the stored value ('Full Board' -> full_board). But the page has no
-- i18n for meal types, so on the Japanese UI it shows English; and the seed left
-- label_ja NULL. Filling label_ja (from rates.meals.* i18n) makes the Japanese
-- UI show Japanese meal names — a fix, not a regression. English is unchanged.
--
-- Built from 20260913 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on the meal_type
--      block (keys + English labels unchanged).
--   2. Backfills existing orgs' meal_type label_ja where blank — preserving
--      any customization.
--
-- Relabel-only: meal_type keys are frozen; this supplies labels.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = '朝食'
    WHERE kind = 'meal_type' AND key = 'breakfast' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ブランチ'
    WHERE kind = 'meal_type' AND key = 'brunch' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '昼食'
    WHERE kind = 'meal_type' AND key = 'lunch' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '夕食'
    WHERE kind = 'meal_type' AND key = 'dinner' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '軽食'
    WHERE kind = 'meal_type' AND key = 'snack' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ハーフボード'
    WHERE kind = 'meal_type' AND key = 'half_board' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フルボード'
    WHERE kind = 'meal_type' AND key = 'full_board' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: all seven meal types must be bilingual after the backfill.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind = 'meal_type'
    AND key IN ('breakfast','brunch','lunch','dinner','snack','half_board','full_board')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'meal_type relabel: % meal types without a Japanese label', no_ja;
  END IF;
END
$verify$;
