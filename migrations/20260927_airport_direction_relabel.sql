-- 20260927_airport_direction_relabel.sql
-- Bilingual backfill for the airport_direction vocabulary.
--
-- Keys match the airport-service rate form (DIRECTIONS = arrival/departure/
-- both; the engine selects by key), rendered via rates.airportServices.
-- directions i18n. Two English notes:
--   * arrival/departure English already matches the app.
--   * the 'both' seed label was 'Both directions' but the app i18n shows
--     'Both' — so this RECONCILES the seed default to 'Both' (surgical, only
--     where an org is still at the old default) so the vocabulary reproduces
--     today's UI. Japanese comes from that same i18n block.
--
-- Built from 20260926 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label + label_ja on the block.
--   2. Fixes the 'both' default label and backfills label_ja where blank —
...
-- Fix the 'both' default label to match the app UI (only where still the old
-- seed default), then backfill Japanese where blank — preserving customization.
  UPDATE org_vocabularies SET label = 'Both'
    WHERE kind = 'airport_direction' AND key = 'both' AND label = 'Both directions';
  UPDATE org_vocabularies SET label_ja = '到着'
    WHERE kind = 'airport_direction' AND key = 'arrival' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '出発'
    WHERE kind = 'airport_direction' AND key = 'departure' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '両方'
    WHERE kind = 'airport_direction' AND key = 'both' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded airport_direction row must be bilingual after the
...
BEGIN
  ...
  WHERE kind = 'airport_direction'
    AND key IN ('arrival','departure','both')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'airport_direction relabel: % directions without a Japanese label', no_ja;
  END IF;
END
$verify$;
