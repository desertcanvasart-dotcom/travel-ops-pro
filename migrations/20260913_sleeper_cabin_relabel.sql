-- 20260913_sleeper_cabin_relabel.sql
-- Non-regressive relabel of the sleeper_cabin vocabulary + bilingual backfill.
--
-- The two sleeper cabins the operator sells are stored as the human strings
-- 'Single' / 'Half Twin' (sleeping_train_rates.cabin_type, free text — see
-- lib/rates/sleeping-train-cabins.ts). The vocabulary keys are the slugs
-- single / half_twin, which useVocabLabel now reaches by slugifying the stored
-- value ('Half Twin' -> half_twin). But the 20260908 seed labelled them
-- "Single Cabin"/"Half Twin Cabin", while the UI shows "Single"/"Half Twin"
-- (rates.sleepingTrains.cabins) — wiring alone would regress EN; label_ja was
-- also NULL.
--
-- Built from 20260912 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with sleeper_cabin labels matching
--      today's UI + label_ja from the Japanese i18n.
--   2. Surgically updates existing orgs (labels only where still the old
--      default; label_ja only where blank) — preserving customization.
--
-- Relabel-only: cabin keys are frozen; this only supplies labels.
...
-- Fix existing orgs' sleeper_cabin labels, preserving customization.
  UPDATE org_vocabularies SET label = 'Half Twin'
    WHERE kind = 'sleeper_cabin' AND key = 'half_twin' AND label = 'Half Twin Cabin';
  UPDATE org_vocabularies SET label = 'Single'
    WHERE kind = 'sleeper_cabin' AND key = 'single' AND label = 'Single Cabin';

-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = 'ハーフツイン'
    WHERE kind = 'sleeper_cabin' AND key = 'half_twin' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'シングル'
    WHERE kind = 'sleeper_cabin' AND key = 'single' AND coalesce(btrim(label_ja), '') = '';
...
BEGIN
  ...
  WHERE kind = 'sleeper_cabin' AND label IN ('Single Cabin','Half Twin Cabin');
  IF old_left > 0 THEN
    RAISE EXCEPTION 'sleeper_cabin relabel: % rows still carry the old "... Cabin" label', old_left;
  END IF;

  SELECT count(*) INTO no_ja
  FROM org_vocabularies
  WHERE kind = 'sleeper_cabin' AND key IN ('single','half_twin')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'sleeper_cabin relabel: % cabins without a Japanese label', no_ja;
  END IF;
END
$verify$;
