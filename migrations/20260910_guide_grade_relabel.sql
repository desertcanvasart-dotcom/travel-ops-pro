-- 20260910_guide_grade_relabel.sql
-- Non-regressive relabel of the guide_grade vocabulary + bilingual backfill.
--
-- The 20260908 seed labelled the 'senior' grade "Senior guide", but this app
-- has always shown "Senior Egyptologist" (rates.guides.guideTypes.senior).
-- Wiring guide grade to the vocabulary would therefore REGRESS the English UI
-- unless the seeded label matches. It also left label_ja NULL for every grade.
--
-- This migration:
--   1. CREATE OR REPLACEs seed_org_vocabulary (built from 20260909 so the
--      transport fix is retained) with the guide_grade block corrected:
--      senior -> "Senior Egyptologist", and label_ja filled from the existing
--      Japanese i18n for the six grades that have one.
--   2. Surgically UPDATEs existing orgs: fixes the 'senior' label only where it
--      is still the old default, and backfills label_ja only where blank — so
--      any operator customization is preserved (no delete/re-seed).
--
-- Relabel-only: the guide_type keys are frozen; this only supplies labels.
...
-- Fix existing orgs' guide_grade labels, preserving any customization.
UPDATE org_vocabularies SET label = 'Senior Egyptologist'
  WHERE kind = 'guide_grade' AND key = 'senior' AND label = 'Senior guide';

  UPDATE org_vocabularies SET label_ja = 'エジプト学者'
    WHERE kind = 'guide_grade' AND key = 'egyptologist' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'シニアエジプト学者'
    WHERE kind = 'guide_grade' AND key = 'senior' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '公認ガイド'
    WHERE kind = 'guide_grade' AND key = 'licensed' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ローカルガイド'
    WHERE kind = 'guide_grade' AND key = 'local' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '専門ガイド'
    WHERE kind = 'guide_grade' AND key = 'specialist' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ドライバーガイド'
    WHERE kind = 'guide_grade' AND key = 'driver_guide' AND coalesce(btrim(label_ja), '') = '';
...
DECLARE
  ...
  no_ja INTEGER;
BEGIN
  -- no org should still carry the old 'Senior guide' default
  SELECT count(*) INTO bad
  FROM org_vocabularies
  WHERE kind = 'guide_grade' AND key = 'senior' AND label = 'Senior guide';
  IF bad > 0 THEN
    RAISE EXCEPTION 'guide_grade relabel: % senior rows still labelled "Senior guide"', bad;
  END IF;

  -- the two grades this app sells must be bilingual after the backfill
  SELECT count(*) INTO no_ja
  FROM org_vocabularies
  WHERE kind = 'guide_grade' AND key IN ('egyptologist','senior')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'guide_grade relabel: % active grades without a Japanese label', no_ja;
  END IF;
END
$verify$;
