-- 20260912_cruise_cabin_relabel.sql
-- Non-regressive relabel of the cruise_cabin vocabulary + bilingual backfill.
--
-- The cruise_cabin keys already match the app (CABIN_TYPES = standard/deluxe/
-- suite), but the 20260908 seed labelled them "Standard Cabin"/"Deluxe Cabin",
-- while this app shows "Standard"/"Deluxe"/"Suite" (rates.cruises.cabinTypes).
-- Wiring alone would REGRESS the English UI; label_ja was also NULL.
--
-- Built from 20260911 so transport + guide + board fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with cruise_cabin labels matching
--      today's UI + label_ja from the Japanese i18n.
--   2. Surgically updates existing orgs (labels only where still the old
--      default; label_ja only where blank) — preserving any customization.
--
...
-- Fix existing orgs' cruise_cabin labels, preserving customization.
  UPDATE org_vocabularies SET label = 'Standard'
    WHERE kind = 'cruise_cabin' AND key = 'standard' AND label = 'Standard Cabin';
  UPDATE org_vocabularies SET label = 'Deluxe'
    WHERE kind = 'cruise_cabin' AND key = 'deluxe' AND label = 'Deluxe Cabin';

-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = 'スタンダード'
    WHERE kind = 'cruise_cabin' AND key = 'standard' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'デラックス'
    WHERE kind = 'cruise_cabin' AND key = 'deluxe' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'スイート'
    WHERE kind = 'cruise_cabin' AND key = 'suite' AND coalesce(btrim(label_ja), '') = '';
...
BEGIN
  ...
  WHERE kind = 'cruise_cabin' AND label IN ('Standard Cabin','Deluxe Cabin');
  IF old_left > 0 THEN
    RAISE EXCEPTION 'cruise_cabin relabel: % rows still carry the old "... Cabin" label', old_left;
  END IF;

  SELECT count(*) INTO no_ja
  FROM org_vocabularies
  WHERE kind = 'cruise_cabin' AND key IN ('standard','deluxe','suite')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'cruise_cabin relabel: % cabins without a Japanese label', no_ja;
  END IF;
END
$verify$;
