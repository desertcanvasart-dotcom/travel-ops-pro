-- 20260926_hotel_kinds_relabel.sql
-- Bilingual backfill for the two hotel vocabularies:
--   hotel_property_type, hotel_supplement.
--
-- hotel_property_type keys + English already match the hotels rate form's
-- property-type <option>s, rendered via rates.hotels.propertyTypes i18n (which
-- supplies the Japanese here). hotel_supplement is a seeded kind with NO app
-- surface yet (no rate column, no form) — it is only visible in the
-- Settings → Vocabulary editor; its Japanese is the conventional term
-- (operator-editable) so that editor reads bilingually. English is unchanged.
--
-- Built from 20260925 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on the two blocks.
...
-- Backfill Japanese labels where blank — preserving any customization.
  -- hotel_property_type (English unchanged; JA from rates.hotels.propertyTypes i18n)
  UPDATE org_vocabularies SET label_ja = 'ホテル'
    WHERE kind = 'hotel_property_type' AND key = 'hotel' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'リゾート'
    WHERE kind = 'hotel_property_type' AND key = 'resort' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'アパートメント'
    WHERE kind = 'hotel_property_type' AND key = 'apartment' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ゲストハウス'
    WHERE kind = 'hotel_property_type' AND key = 'guesthouse' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'クルーズ'
    WHERE kind = 'hotel_property_type' AND key = 'cruise' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'キャンプ'
    WHERE kind = 'hotel_property_type' AND key = 'camp' AND coalesce(btrim(label_ja), '') = '';

  -- hotel_supplement (no app surface yet; JA conventional, operator-editable)
  UPDATE org_vocabularies SET label_ja = 'ナイルビュー'
    WHERE kind = 'hotel_supplement' AND key = 'view_nile' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'シービュー'
    WHERE kind = 'hotel_supplement' AND key = 'view_sea' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ピラミッドビュー'
    WHERE kind = 'hotel_supplement' AND key = 'view_pyramid' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ガーデンビュー'
    WHERE kind = 'hotel_supplement' AND key = 'view_garden' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'プールビュー'
    WHERE kind = 'hotel_supplement' AND key = 'view_pool' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '高層階'
    WHERE kind = 'hotel_supplement' AND key = 'upper_floor' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ハーフボード（HB）'
    WHERE kind = 'hotel_supplement' AND key = 'half_board' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フルボード（FB）'
    WHERE kind = 'hotel_supplement' AND key = 'full_board' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'オールインクルーシブ（AI）'
    WHERE kind = 'hotel_supplement' AND key = 'all_inclusive' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ウルトラオールインクルーシブ（UAI）'
    WHERE kind = 'hotel_supplement' AND key = 'ultra_all_inclusive' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded row of the two hotel kinds must be bilingual after
-- the backfill (rows an operator added themselves are exempt).
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE (
      (kind = 'hotel_property_type' AND key IN ('hotel','resort','apartment','guesthouse','cruise','camp'))
   OR (kind = 'hotel_supplement' AND key IN ('view_nile','view_sea','view_pyramid','view_garden','view_pool','upper_floor','half_board','full_board','all_inclusive','ultra_all_inclusive'))
    )
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'hotel kinds relabel: % hotel rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
