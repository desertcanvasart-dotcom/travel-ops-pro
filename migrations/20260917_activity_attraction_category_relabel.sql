-- 20260917_activity_attraction_category_relabel.sql
-- Bilingual backfill for the activity_category and attraction_category vocabs.
--
-- Both kinds' keys + English labels already match the app (activity: ACTIVITY_
-- CATEGORIES raw strings, slugify-reached; attraction: categoryOptions slugs
-- rendered via i18n). The seed left label_ja NULL. The Japanese labels here
-- come straight from the app's own i18n (rates.activities.categories and
-- rates.attractions.categories), so nothing is invented and English is
-- unchanged. For the activity page — which renders the raw English string with
-- no i18n — this additionally lets the Japanese UI show Japanese.
--
-- Built from 20260916 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on both blocks.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = '古代遺跡'
    WHERE kind = 'activity_category' AND key = 'ancient_sites' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '博物館'
    WHERE kind = 'activity_category' AND key = 'museums' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '砂漠サファリ'
    WHERE kind = 'activity_category' AND key = 'desert_safari' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '水上アクティビティ'
    WHERE kind = 'activity_category' AND key = 'water_activities' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '文化体験'
    WHERE kind = 'activity_category' AND key = 'cultural_experience' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'アドベンチャー'
    WHERE kind = 'activity_category' AND key = 'adventure' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '宗教施設'
    WHERE kind = 'activity_category' AND key = 'religious_sites' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '自然・野生動物'
    WHERE kind = 'activity_category' AND key = 'nature_wildlife' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エンターテイメント'
    WHERE kind = 'activity_category' AND key = 'entertainment' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ショッピングツアー'
    WHERE kind = 'activity_category' AND key = 'shopping_tours' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ショー・イベント'
    WHERE kind = 'activity_category' AND key = 'shows_events' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ナイル川体験'
    WHERE kind = 'activity_category' AND key = 'nile_experience' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '現地交通'
    WHERE kind = 'activity_category' AND key = 'local_transport' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '神殿'
    WHERE kind = 'attraction_category' AND key = 'temple' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ピラミッド'
    WHERE kind = 'attraction_category' AND key = 'pyramid' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '博物館'
    WHERE kind = 'attraction_category' AND key = 'museum' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '墓'
    WHERE kind = 'attraction_category' AND key = 'tomb' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '教会'
    WHERE kind = 'attraction_category' AND key = 'church' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'モスク'
    WHERE kind = 'attraction_category' AND key = 'mosque' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '要塞'
    WHERE kind = 'attraction_category' AND key = 'fortress' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '宮殿'
    WHERE kind = 'attraction_category' AND key = 'palace' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '自然'
    WHERE kind = 'attraction_category' AND key = 'nature' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エンターテイメント'
    WHERE kind = 'attraction_category' AND key = 'entertainment' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'その他'
    WHERE kind = 'attraction_category' AND key = 'other' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded row of both kinds must be bilingual.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind IN ('activity_category','attraction_category')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'activity/attraction category relabel: % rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
