-- 20260928_airline_relabel.sql
-- Bilingual backfill for the airline vocabulary (Japanese carrier names).
--
-- NOTE ON SCOPE: airline has NO display surface to wire. On a flight rate the
-- airline is a SUPPLIER (SupplierPicker, air-carrier role) and its IATA code
-- comes from lib/airline-codes.ts — the flight form never reads this
-- vocabulary. The airline kind is consumed ONLY by the Settings → Vocabulary
-- editor (which edits each carrier's name + meta.code). So, like
-- hotel_supplement, this is a Settings-editor-only kind: nothing is wired in
-- the app, and this migration only supplies label_ja so that editor reads
-- bilingually. The keys and the meta.code IATA map are unchanged; English is
-- unchanged; the Japanese carrier names are the conventional ones and are
-- operator-editable.
--
-- Built from 20260927 so all prior fixes are retained.
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja on the block
--      (the meta.code column is preserved).
...
-- Backfill Japanese carrier names where blank — preserving customization.
  UPDATE org_vocabularies SET label_ja = 'エジプト航空'
    WHERE kind = 'airline' AND key = 'egyptair' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ナイル・エア'
    WHERE kind = 'airline' AND key = 'nile_air' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エア・カイロ'
    WHERE kind = 'airline' AND key = 'air_cairo' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フライドバイ'
    WHERE kind = 'airline' AND key = 'flydubai' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エミレーツ航空'
    WHERE kind = 'airline' AND key = 'emirates' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'カタール航空'
    WHERE kind = 'airline' AND key = 'qatar_airways' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'トルコ航空'
    WHERE kind = 'airline' AND key = 'turkish_airlines' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ルフトハンザ航空'
    WHERE kind = 'airline' AND key = 'lufthansa' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ブリティッシュ・エアウェイズ'
    WHERE kind = 'airline' AND key = 'british_airways' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エールフランス航空'
    WHERE kind = 'airline' AND key = 'air_france' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'KLMオランダ航空'
    WHERE kind = 'airline' AND key = 'klm' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エティハド航空'
    WHERE kind = 'airline' AND key = 'etihad' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'サウジアラビア航空'
    WHERE kind = 'airline' AND key = 'saudia' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ロイヤル・ヨルダン航空'
    WHERE kind = 'airline' AND key = 'royal_jordanian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ミドル・イースト航空'
    WHERE kind = 'airline' AND key = 'middle_east_airlines' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'エア・アラビア'
    WHERE kind = 'airline' AND key = 'air_arabia' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'その他'
    WHERE kind = 'airline' AND key = 'other' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded airline row must be bilingual after the backfill
-- (rows an operator added themselves are exempt).
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind = 'airline'
    AND key IN ('egyptair','nile_air','air_cairo','flydubai','emirates','qatar_airways','turkish_airlines','lufthansa','british_airways','air_france','klm','etihad','saudia','royal_jordanian','middle_east_airlines','air_arabia','other')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'airline relabel: % carriers without a Japanese label', no_ja;
  END IF;
END
$verify$;
