-- 20260925_guide_kinds_relabel.sql
-- Bilingual backfill for the two remaining guide vocabularies:
--   guide_duration, guide_language.  (guide_grade was 20260910.)
--
-- Keys + English labels already match the app: guide_duration keys are the
-- guide rate page's TOUR_DURATIONS values, rendered via rates.guides.
-- tourDurations i18n (which supplies the Japanese here); guide_language stores
-- the human language string ('English'), slugify-reached to the seed key, and
-- has no app i18n, so the Japanese is the conventional language name
-- (operator-editable). English is unchanged.
--
-- Built from 20260924 so all prior fixes are retained.
...
-- Backfill Japanese labels where blank — preserving any customization.
  -- guide_duration (English unchanged; JA from rates.guides.tourDurations i18n)
  UPDATE org_vocabularies SET label_ja = '半日（4時間）'
    WHERE kind = 'guide_duration' AND key = 'half_day' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '終日（8時間）'
    WHERE kind = 'guide_duration' AND key = 'full_day' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '送迎・アシストのみ'
    WHERE kind = 'guide_duration' AND key = 'meet_greet' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '延長（10時間以上）'
    WHERE kind = 'guide_duration' AND key = 'extended' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '時間単位'
    WHERE kind = 'guide_duration' AND key = 'hourly' AND coalesce(btrim(label_ja), '') = '';

  -- guide_language (English unchanged; JA = conventional language name, operator-editable)
  UPDATE org_vocabularies SET label_ja = '英語'
    WHERE kind = 'guide_language' AND key = 'english' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'アラビア語'
    WHERE kind = 'guide_language' AND key = 'arabic' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フランス語'
    WHERE kind = 'guide_language' AND key = 'french' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ドイツ語'
    WHERE kind = 'guide_language' AND key = 'german' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'スペイン語'
    WHERE kind = 'guide_language' AND key = 'spanish' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'イタリア語'
    WHERE kind = 'guide_language' AND key = 'italian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ロシア語'
    WHERE kind = 'guide_language' AND key = 'russian' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '中国語'
    WHERE kind = 'guide_language' AND key = 'chinese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '日本語'
    WHERE kind = 'guide_language' AND key = 'japanese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ポルトガル語'
    WHERE kind = 'guide_language' AND key = 'portuguese' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'オランダ語'
    WHERE kind = 'guide_language' AND key = 'dutch' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ポーランド語'
    WHERE kind = 'guide_language' AND key = 'polish' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: every seeded row of the two guide kinds must be bilingual after
...
BEGIN
  ...
  WHERE (
      (kind = 'guide_duration' AND key IN ('half_day','full_day','meet_greet','extended','hourly'))
   OR (kind = 'guide_language' AND key IN ('english','arabic','french','german','spanish','italian','russian','chinese','japanese','portuguese','dutch','polish'))
    )
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'guide kinds relabel: % guide rows without a Japanese label', no_ja;
  END IF;
END
$verify$;
