-- 20260911_board_basis_relabel.sql
-- Bilingual backfill for the board_basis vocabulary.
--
-- The 20260908 seed left label_ja NULL for every board type. The keys already
-- match the app once case is normalised: vocabulary keys are lowercase by
-- constraint (org_vocabularies_key_check), while hotels store UPPERCASE codes
-- ('BB','HB',…) — useVocabLabel lowercases the stored code before matching, so
-- no key change is needed (and none is possible: uppercase keys fail the CHECK).
--
-- This migration (built from 20260910 so transport + guide fixes are retained):
--   1. CREATE OR REPLACEs seed_org_vocabulary with label_ja added to the
--      board_basis block (keys unchanged, lowercase).
--   2. Backfills existing orgs' board_basis label_ja where blank — preserving
--      rank / is_active / any label customization.
--
-- Relabel-only: the stored board_basis codes are frozen; this supplies labels.
...
-- Backfill Japanese labels where blank.
  UPDATE org_vocabularies SET label_ja = '素泊まり'
    WHERE kind = 'board_basis' AND key = 'ro' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = '朝食付き'
    WHERE kind = 'board_basis' AND key = 'bb' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'ハーフボード'
    WHERE kind = 'board_basis' AND key = 'hb' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'フルボード'
    WHERE kind = 'board_basis' AND key = 'fb' AND coalesce(btrim(label_ja), '') = '';
  UPDATE org_vocabularies SET label_ja = 'オールインクルーシブ'
    WHERE kind = 'board_basis' AND key = 'ai' AND coalesce(btrim(label_ja), '') = '';

-- Self-verify: the five board types must be bilingual after the backfill.
DO $verify$
DECLARE
  no_ja INTEGER;
BEGIN
  ...
  WHERE kind = 'board_basis' AND key IN ('ro','bb','hb','fb','ai')
    AND coalesce(btrim(label_ja), '') = '';
  IF no_ja > 0 THEN
    RAISE EXCEPTION 'board_basis relabel: % board types without a Japanese label', no_ja;
  END IF;
END
$verify$;
